...
const MESH_ONLY_FILES: &[(&str, &str)] = &[
    // nist_ctc_02: 0.0386 mm of authored curve/surface gap — four times
    // MAX_ALLOWED_TOLERANCE — keeps it off the exact path for good, and
    // rightly (of-bb6 measures it; refusing a tolerance the kernel cannot
    // give honestly is the point). What changed is the degrade: of-05ac
    // closes the mesh fallback over its trimmed faces (0 open edges and 0
    // pinched, down from 15843 and 14569), and of-aoml gives its tilted-bore
    // rim lenses corners of their own so the rim vertices carry
    // pseudonormals `MeshSdf::new` accepts. The solid arrives as a mesh
    // instead of vanishing.
    ("nist/nist_ctc_02_asme1_rc.stp", "of-bb6"),
];
