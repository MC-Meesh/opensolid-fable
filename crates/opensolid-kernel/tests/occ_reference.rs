...
const MESH_ONLY_FILES: &[(&str, &str)] = &[
    // Empty today: bspline_patch_prism sat here while its extrusion patch was
    // mis-sized (of-8ulj); sizing the patch to its face put it back on the
    // exact path, where it is measured below. The machinery stays — the next
    // solid the exact path refuses lands here instead of vanishing (of-05ac).
];
