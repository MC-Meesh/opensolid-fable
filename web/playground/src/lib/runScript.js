 ...
 *
 * The script body runs in strict mode with two bindings: `Shape` and
 * `Profile` (the 2D profile builder consumed by `Shape.extrude` /
 * `Shape.revolve`). Throws on syntax errors, runtime errors, or a non-Shape
 * return value; the thrown error's message is suitable for direct display.
 *
 ...
 */
export function runScript(source, ShapeClass, ProfileClass) {
  const build = new Function('Shape', 'Profile', `"use strict";\n${source}`);
  const shape = build(ShapeClass, ProfileClass);
